...
import { Trash2, UserPlus, Loader2, Copy, RefreshCw, AlertTriangle } from "lucide-react";
              ...
              <div className="flex flex-col gap-2">
                <div className="flex gap-2">
                    <Input 
                        ...
                        className="rounded-xl h-12 bg-zinc-100 font-mono text-sm flex-1 cursor-not-allowed" 
                    />
                    <Button type="button" variant="outline" size="icon" onClick={handleCopyPassword} disabled={!formData.password} className="h-12 w-12 rounded-xl shrink-0 border-violet-100 text-violet-600 hover:bg-violet-50">
                        <Copy size={18} />
                    </Button>
                </div>
                <Button type="button" onClick={handleGeneratePassword} className="w-full bg-zinc-900 hover:bg-black text-white h-10 rounded-xl text-xs font-bold gap-2">
            ...
            {formData.password && (
                <div className="bg-amber-50 border border-amber-100 p-4 rounded-2xl flex gap-3 animate-in fade-in slide-in-from-bottom-2 duration-300">
                    ...
                    <div className="text-[11px] leading-tight text-amber-900">
                        ...
                        <p className="opacity-80">The password is visible above. Copy it now and provide it to the user. <strong>It will not be stored in plain text.</strong></p>
                    </div>
                </div>
            )}

            <Button type="submit" disabled={isLoading || !formData.password} className="w-full bg-violet-600 hover:bg-violet-700 h-12 rounded-xl mt-2 font-bold shadow-lg shadow-violet-200 transition-all active:scale-[0.98]">
                ...
                users.map((u: any) => (
                    <TableRow key={u.id} className="group hover:bg-violet-50/20 transition-colors">
                      ...
                      <TableCell className="text-right pr-8">
                        ...
                        {/* 🚨 UPDATED DELETE FLOW WITH ALERT DIALOG 🚨 */}
                        <AlertDialog>
